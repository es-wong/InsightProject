...
# open weights arrays
with open('weight_array.pickle', 'rb') as file:
...
def model_output(user_select,NameVec,featureDimRed,tSNE_vecs,imagePathList,weights):
    ...
    plt.savefig(staticDir + '/' + figName + '_' + np.array2string(userIdx[0]) + '.png', dpi = 250, bbox_inches = 'tight')

    figPath = figName + '_' + np.array2string(userIdx[0]) + '.png'
    ...
    # need to pass the userIdx string (as a list...?)
    figPathList = []
    figPathList.append(figPath)

    print("FIGPATHLIST ------->", figPathList[0])

    return(nameList, scoreList, imgList2, inputPath2, figPathList)
